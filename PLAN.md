...
- **Libraries:** none for parsing (JSON; `DOMParser` for CycloneDX XML); `ajv` (lazy) with bundled CycloneDX 1.2–1.6 and SPDX 2.3 JSON schemas.
...
- **Implementation notes (2026-09-05):** schemas are committed under `src/tools/sbom-viewer/schemas/` (pinned commits + SHA-256 in `SOURCES.md`) and loaded as one lazy chunk per spec version inside a Comlink worker; `vite.config.ts` sets `worker: { format: 'es' }` so worker chunks can code-split (every worker in the app is already a module worker). CycloneDX XML is parsed on the main thread (workers have no `DOMParser`) and is not schema-validated. Ajv's `uniqueItems` on object arrays is O(n²) (11 s for 10k components), so it is stripped from object-typed arrays at compile time and duplicates in components/services/dependencies/vulnerabilities are detected in O(n) instead (~50 ms for 10k).
...
- **Implementation notes (2026-09-05):** v4.0 is a TypeScript port of FIRST's reference `cvss_score.js` with the lookup / max-composed / max-severity tables vendored verbatim (BSD-2-Clause, pinned commit in `src/tools/cvss-calculator/v4/SOURCES.md`). One deliberate deviation: the reference ends in `Math.round(value * 10) / 10`, which turns a true 4.95 (arriving as 4.9499999…) into 4.9; the port snaps to six decimals first and rounds half-up (5.0), matching FIRST's published tables and decimal-based implementations. Fixtures: 35 official v3.1/v4.0 example pairs from FIRST's v4.0 Examples document plus 400 random vectors per version scored with the Red Hat `cvss` Python library, which reproduces all official values; every fixture value is asserted in `fixtures.test.ts`. The v3.1 environmental score can legitimately differ from the base score by 0.1 with no environmental metrics set when Scope is Changed (different exponent in the modified-impact formula) — do not "fix" it.
...
- **Implementation notes (2026-09-05):** pdf-lib only, loaded with `updateMetadata: false` (its default stamps a pdf-lib Producer/dates — the opposite of a sanitizer); the Info dictionary is reached through `context.trailerInfo` because `getInfoDict()` is private, and every value is written as UTF-16 text by hand (pdf-lib's `setKeywords` rewrites punctuation). XMP is parsed with `DOMParser` instead of pdf.js (plain XML; no 1 MB dependency for a text view) and regenerated packets carry only the fields the user entered. "Strip all" also removes page/XObject XMP streams, PieceInfo, resets `/ID`, and — found in verification — **deletes every object unreachable from the trailer**: pdf-lib writes back the orphaned objects of earlier incremental revisions, so a replaced Info dictionary otherwise survives inside a compressed object stream where `grep` and exiftool cannot see it (fixture `incremental-update.pdf`; check such leaks by enumerating all xref objects with pypdf). Signature fields trigger an invalidation warning. Verified with exiftool 13.59, poppler `pdfinfo`, pypdf and pdf.js in Node. Handout: `docs/handouts/phase-58-pdf-metadata.md`.
...
- **Implementation notes (2026-09-06):** shipped in three commits (parts A/B/C) plus a verification fix-up. *PWA:* `vite-plugin-pwa` 1.3.0 (generateSW, prompt mode) with a shell-only precache (51 entries ≈ 560 KiB: `index.html`, entry + `/tools` route chunks and their static imports, CSS, fonts, icons) computed by a build-time plugin (`scripts/offlineAssets.ts`) that also emits `offline-assets.json` (615 files, 42 MB); runtime caching for hashed tool chunks (`nymbx-assets`, CacheFirst), the background-removal shards, the OCR engine/language packs and the un-hashed zxing/Noto files (`ignoreVary` is required — module-script requests carry `Origin`, the warm-up fetch does not); `clientsClaim` must be set explicitly or the first visit stays uncontrolled. The SW registers lazily after `load`; updates surface as a toast with a Reload action; a `ChunkErrorBoundary` replaces the blank page for an uncached route; the dashboard's "Use NYMBX offline" panel shows cache coverage and offers "Download all tools" (`prefetchUrls`, ~5 s warm). Icons, `favicon.ico`, Open Graph/Twitter meta, `theme-color`, `robots.txt`, and `no-cache` for the shell/SW in the Caddyfile. *Dashboard:* ranked search over name/keywords (new `ToolMeta.keywords`, 5–6 per tool)/category/description with `?q=`/`?category=` URL sync (debounced — writing the URL per keystroke dropped characters under 20 ms apart), category chips, arrow/Enter navigation; global shortcuts (Ctrl/⌘ K, `/`, `?` help dialog, Shift+D, Esc) in the toolbox shell; theme as an external store. *Contrast:* token-only fix — light `--c-pine #247a55`, `--c-pine-deep #1d6a4b`, `--c-amber #8c5214`, `--c-muted #626267`, `--c-faint #70707a`; dark `--c-muted #a3a3aa`, `--c-faint #8e8e97` — all 40 used pairs ≥ 4.5:1, axe `color-contrast` clean in both themes; a11y follow-ups (`inert` mobile drawer, dropzone and tool cards named by their visible content with the file inputs outside the button, CodeMirror `aria-label`s) take Lighthouse accessibility to 100 on every measured page. *Cross-browser:* `@playwright/test` 1.62.1 with chromium/firefox/webkit/mobile-webkit projects (`npm run e2e`; WebKit needs `PLAYWRIGHT_SKIP_VALIDATE_HOST_REQUIREMENTS=1` on this machine), 58 tests green, no browser-specific defects found across all 54 tools. *Performance:* password-strength defers the zxcvbn dictionaries to the first keystroke (route chunk 1.67 MB → 7.7 kB; mobile Lighthouse 73 → ~88). Lighthouse (desktop/mobile): `/tools` 100/93 perf, 100/96 a11y; `/tools/password-strength` 100/88, 100/96; `/tools/pdf-sign-annotate` 100/89, 95/91. Entry chunk 265,995 → 270,090 B (boundary + theme store). Open: mobile performance on markdown-editor (64–74, 4.7 MB on load — CodeMirror + shiki) and the two heaviest PDF tools sits below 90; `nymbx-assets` `maxEntries: 1500` ≈ two deploys of assets before LRU eviction of a full offline download. Handout: `docs/handouts/phase-61-polish-pwa.md`.
